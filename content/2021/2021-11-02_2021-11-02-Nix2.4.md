...
# ちゃんと理解してflake.nixで自分の環境を構築する(2021-11-06)

[これ](https://blog.ysndr.de/posts/internals/2021-01-01-flake-ification/)見て理解した。
...
# flake.nix
{
  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system: {
      let
         emacs-head = pkgs.callPackage ./emacs-head/flake.nix { inherit pkgs; };
         kissat     = pkgs.callPackage ./kissat/falke.nix     { inherit pkgs; };
         sat-bench  = pkgs.callPackage ./sat-bench/flake.nix  { inherit pkgs; };
         ...
      in
        {
           packages = { emacs-head; kissat; sat-bench; ... };
           # defaultPackage = ...;
           overlay = [];
        }
    }
